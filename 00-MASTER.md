...
- `floema-theme/` ← Nuxt SSG + Three.js 站 `www.floema.com`（2026-05-18）
- `viture-beast-theme/` ← React + GSAP 站（2026-05-20）
- `crav-theme/` ← Next.js 15 + Turbopack + Tailwind v4 + GSAP 站 `cravburgers.shop`（2026-05-25）

> 本文是**入口和总览**。深入细节请按章节链接跳转到 `01-`~`09-` 章节文件和 `BUGS.md`。

---

## ⚠️ 文件归档铁律（2026-05-25 加入）

**所有移植项目的踩坑总结、教训、动效 spec、新发现的母版陷阱 → 一律写到这个 `playbook/` 文件夹**，不要散落到其他位置（项目根目录的 README、theme 内的 notes、临时 md 文件等）。

- 一次性 bug → 追加到 [BUGS.md](BUGS.md)，沿用 `PN-NN` 编号
- 新动效模式 → 追加到 [03-animations.md](03-animations.md) 或 [08-nextjs-porting.md](08-nextjs-porting.md) §8（按源站技术栈分）
- CSS cascade 新陷阱 → [02-css-cascade.md](02-css-cascade.md)
- 部署/CLI 新坑 → [04-shopify-deploy.md](04-shopify-deploy.md)
- 资源抓取新技巧 → [01-architecture.md](01-architecture.md) 或 [08-nextjs-porting.md](08-nextjs-porting.md) §2
- 自验方法 → [09-playwright-verify.md](09-playwright-verify.md)
- 新发现的元规则 → 本文档 §10

理由：散落写在多个地方时下次接手项目找不全。一个文件夹 single-source-of-truth。Claude 的 memory 只放索引指针，长篇内容必入 playbook。
...
**默认走 Phase 2 多 section + blocks**。block 划分的标准做法见 §10.7 模式 C。
...
### 10.7 1:1 复刻为什么总要 2~3 轮修改（2026-05-27 反思）

SUSOS product + collection 两次部署都出现"用户说和原版一样"→ "推完发现还差很多" 的循环。深挖根因后归纳成 6 个高频模式，每个都给出 actionable 防御。

**模式 A：只看 desktop 截图，没切 mobile viewport**
- 病：mobile carousel padding、mobile drawer、mobile 搜索框、mobile filter 按钮样式 — 全是 mobile 专属，desktop 跑不出来
- 防御：每次写完 section 后 LIVE 跑 Playwright 三档 viewport：`1440×900` / `768×1024` / `390×844`，每档 4 角截图（top/mid/3-quarter/bot）

**模式 B：值靠"推断"，不靠 grep**
- 病：CSS variable `clamp(20px, 4vw, 80px)` 推断成 `20px` 硬编码；swatch slug `pearl/ivory/stone` 推断成 `pearl-white/warm-ivory/stone-grey`；padding 推断成 `28px` 而源版是 0
- 防御：复刻任何 CSS 值前 **必须 grep 源 HTML / CSS** 拿真实值，不允许"看着差不多"

**模式 C：拿到 demo source 后没自主设计 block schema 就开写**
- 病：用户给了 `designs/{NNN}/index.html` 后我直接开始写 section，没先通读 source 划分"哪些是 section 边界 / 哪些是 block 边界 / 哪些是 setting"
- 防御：开写代码前**必须**通读 source 一遍，列出：
  1. 顶层 section 切分（按 source 里 `<section>` / `<main>` 标签 + 视觉断点）
  2. 每个 section 内哪些元素是"重复的同类列表"（trust items / pillars / cards / faq / steps）→ 这些是 **blocks**
  3. 每个 block 内哪些是 admin 要换的（文本 / 图 / URL / 颜色）→ 这些是 **block.settings**
  4. section-level 配置（heading / 是否显示 mid-banner / CTA 文案）→ **section.settings**
- 例：collection 页 18 张卡片 → 18 个 `demo_card` blocks，每张卡 settings 含 image_picker / name / tier / price / moq / badge_text + badge_variant。trustbar 4 项 → 4 个 `item` blocks。不允许 18 张卡硬编码在 liquid 数组里
- 触发判断：**任何"同类、可枚举"且超过 2 个的元素** → 直接 blocks，不要 inline array hack

**模式 D：CSS specificity 战争 + cascade 遗忘**
- 病：新 section 写 `.foo { background: grey }` 单类 0,1,0；不知道 body section 同类 0,1,0 但在 compiled stylesheet 里晚出现就赢了，结果 grey 被刷成 white
- 防御：建新 section 前先 `grep "<class>" sections/` 看是不是已经有同类规则；如果有就用 **double-class** `.outer.inner` 拿 0,2,0 + 必要时 `!important` 双保险
- 已知冲突源：`.susos-collection-body`、`.susos-pdp`、`.page-content`、Lumia 母版的 `body.gradient`

**模式 E："我以为我做对了"，没拿三方对照**
- 病：用户说"原版"，我没把 (a) source HTML 原始截图 (b) himax 单 section preview (c) 当前 susos-2 preview 三个并排比，每次都是"我觉得做对了就 push"
- 防御：报告"完成"前必须 quote/截图三方对照（source / 已知 ok 的 himax / 当前 susos-2），自己肉眼对比无差异才报

**强制 checklist（每次 push 前必跑）：**

1. [ ] grep source CSS 拿全部 padding / margin / color / slug 真实值
2. [ ] Playwright 三档 viewport 各 4 角截图
3. [ ] 跟已知 ok 版本（himax 单 section）做截图对比
4. [ ] admin Theme Editor 验证每个 block 都可编辑（image_picker / text / select 等）
5. [ ] CDP `CSS.getMatchedStylesForNode` 检查关键元素是否被高优先级规则压
6. [ ] CLI push 后 pull verify — 不允许 silent-skip

跳任何一步 → 大概率会有 2nd/3rd round 修改。

### 10.8 文件夹组织铁律（2026-05-27 加入）

**一个项目 = 一个本地 theme 文件夹**。不允许同一个项目散布在多个目录里。

- 路径模式：`d:/shopify_try/themes/<name>/<name>-theme/`（如 `ubs/ubs-theme/` / `crav/crav-theme/`），或归到 `_misc/<name>-theme/`
- 一个 store 的多个模板（product / collection / index）共用同一个本地 theme 文件夹，**不要因为新做一个 template 就新开一个文件夹**
- 多 store 之间才允许独立文件夹（如 himax 的 `susos-coll-theme` 跟 susos-2 的 `susos-theme` 各自一份）
- pull / push 时永远在项目根目录下，避免 `cd` 到嵌套子目录

**为什么这是铁律：** SUSOS 项目同时做 product + collection 两个模板，差点又开 `susos-coll-theme` 副本。Shopify CLI 一个文件夹对一个 theme ID 已经足够；同 theme 的不同 template 就是文件夹内 `templates/X.json` 多文件，跟 sections 共享。开新文件夹会重复推 assets + 同步混乱。

**新增模板 / 新 section / 新 asset 时的标准做法：**
- 模板：`templates/<page>.<suffix>.json`，suffix 走业务名（如 `susos-himax`）
- Section：`sections/<scope>-<name>.liquid`，scope 用前缀（`susos-collection-*` / `susos-pdp-*`）
- Asset：`assets/<scope>-<name>.<ext>`，asset 命名严格跟 section 内 `asset_url` 引用对齐
- 永远 `cp` / 编辑而不是新建目录
...
| [02-css-cascade.md](02-css-cascade.md) | Dawn/Lumia 全局污染、specificity 规则（**含 Tailwind v4 @layer 警告**） |
...
| [09-playwright-verify.md](09-playwright-verify.md) | **Playwright 自验方法论**：CDP `CSS.getMatchedStylesForNode` 找赢的规则、逐 section scroll+screenshot、hover/scroll 动效验证 |
| [BUGS.md](BUGS.md) | 已修 bug 目录，含症状/根因/修法/教训（P1/P2/P3/P4 系列） |
...
### floema-theme（Nuxt SSG + Web Worker Three.js，Phase 2）

- 源站 `www.floema.com`，Portuguese 家具品牌 5 个 collection（Urban/Nature/RePlastic/Golf/Details）
- 部署在 himaxlimited.myshopify.com，theme ID **154979106989**，14 个 section（9 个内容 + 5 个 overlay）
- 全项目复杂度顶峰 — Web Worker + Three.js + GLSL shaders + Sanity CDN + Vue3 lazy imports
- 关键踩坑（按时间顺序，**部署阶段 → 内容阶段**）：

  **部署阶段**（卡了 3+ 小时反复 push 都失败 — Shopify CLI silent-skip 一家四口）：
  1. **Schema name >25 字符** push 报错（[P3-07](BUGS.md#p3-07)）：`FL Collections Overview` 23 字符过，`Floema · Collections Overview` 30 字符直接拒。Build 脚本里 truncate
  2. **Section 文件名（type）>25 字符 silent reject**（[P3-07b](BUGS.md#p3-07b)）：`floema-07-workflow-overview` 27 字符，CLI 报 `upload complete` 但服务端没落地，访问首页直接 fallback 404 模板。修法：建 SHORTENINGS 缩写表（`-collections` → `-coll`、`-overview` → `-ovr` 等），文件名压到 ≤25
  3. **CLI bulk push 静默跳过部分新文件**（[P3-07c](BUGS.md#p3-07c)）：14 个 new sections 推完报成功，pull 下来只有 9 个，缺 5 个。逐个单独 push 才上传
  4. **CLI 状态污染后所有单文件 push 全部失效**（[P3-07d](BUGS.md#p3-07d)）：之前一次 `layout could not be deleted` 警告后，CLI 卡在缓存污染态，30+ 次 `--only` push 全部 silent skip。判定标准用 admin Themes 列表 `Last saved` 时间戳（CLI 输出全是假的）。修法：不带 `--only` 的完整 mirror push + `--nodelete`
  5. **CLI checksum cache 死锁**（[P3-07e](BUGS.md#p3-07e)）：改文件内容、加 `!important`、改版本号注释都没用，CLI 死活不上传。修法：**重命名文件**（base → base2 → base3 → ... → base10），CLI 必须把新文件名当新增上传。本项目 base.css 改了 10 次名才搞定
  6. **嵌套子 wrapper 在 Lumia 母版上不可靠**（[P3-08](BUGS.md#p3-08)）：写 `{% if index %}<div class="floema-page">{% endif %}` 包裹 sections，DOM 里看不到这个 div。Lumia 母版的某个 section 输出了破坏性嵌套吞掉了。修法：直接把 `.floema-page` class 合并到永远存在的 `<div class="page-content">` 上

  **内容阶段**（动效逐个复刻，本次 session 的踩坑）：
  7. **数据偷工减料**（[P3-09](BUGS.md#p3-09)）：intro 41 张图我自作主张精选 16 张省 token，用户截图立刻发现
  8. **CSS hint 误导**（[P3-10](BUGS.md#p3-10)）：intro 写了 `.slider-image { offset-path: ellipse }` 60s orbit CSS，按它复刻了 v1/v2/v3 三版都不对。真实机制在 `C4rlixGp.js` lazy chunk 里启 Web Worker 跑 Three.js Z-tunnel
  9. **CSS-死代码追踪**（[P3-14](BUGS.md#p3-14)）：Vue lazy import + Web Worker 把真实动画藏在三层间接里。Worker 文件 4000+ 行 Three.js 库代码，**末尾 ~100 行**才是用户场景。`tail -c 8000` 是必修动作
  10. **Sanity CDN CORS 失败**（[P3-11](BUGS.md#p3-11)）：源站 Sanity URL 给 Three.js TextureLoader 用全部 tainted texture 失败，本地化 + 同源 CDN 才能用
  11. **Liquid→JS 桥**（[P3-12](BUGS.md#p3-12)）：用 JSON script tag 把 `asset_url` 渲染过的 URL 数组传给 JS
  12. **Shader 不是粒子**（[P3-13](BUGS.md#p3-13)）：shadows-portal 看似是叶子粒子，实际是 packed RGBA 纹理 + 自定义 shader 采样不同通道（layer 0=R, 1=G, 3=alpha 反相）
  13. **Smooth-scroll resync**（[P3-15](BUGS.md#p3-15)）：scrollbar drag 释放后回弹，自制 lerp 漏了 external scroll source 的同步
  14. **Overlay section wrapper**（[P3-16](BUGS.md#p3-16)）：modal/drawer/curtain 5 个 section 的 Shopify wrapper 占了 500px+ 空白，footer 下方一大块空，需 `display:contents` 折叠 wrapper

- **部署阶段的元教训**：
  - **CLI 输出 `Theme upload complete` 是常量字符串，跟服务端无关**。判定真伪只看 admin Themes 列表 `Last saved` 时间戳
  - **CLI silent-skip 有 5 种触发条件**：schema name 超长 / 文件名超长 / 某种 bulk push diff bug / 状态污染 / checksum cache 死锁。一律靠**重命名文件**作为终极 escape hatch（floema base.css 改到 base10 才稳）
  - **诊断 banner 是验证 push 是否真生效最快的方法**：往 head 里塞 `<style> body::before { content: "vNN" }</style>`，每次改 N 编号刷 preview 看数字 — 不出现就是 silent skip
  - **build 脚本应同时输出 sections + index.json**，避免 type 名重命名后人工维护漏一边

- **架构提炼**：
  - 14 个 Liquid section，其中 9 个内容（intro / collections-cta / company-highlight / featured-products / collections-overview / leadership-highlight / workflow-overview / lower-tagline / footer）+ 5 个 overlay（option-modal / contact-modal / modal-drawer / grid / loading-curtain）
  - 自制 JS：intro-tunnel（Three.js Z-tunnel，46 planes fibonacci 螺旋）/ shadows-portal（GLSL shader + packed texture）/ collections-cta（GSAP ScrollTrigger 横向 sticky）/ footer-parallax / smooth-scroll / entry-reveal
  - 本地化 assets：41 intro images + 2 shadow textures + Three.js 库 + GSAP plugins
  - CSS 文件：6 个（base / inline-styles / entry / articlecard / revealimage / footer / lowertagline）— 其中 inline-styles 255KB 是从源 head `<style>` 块抽出来的 68 段 Vue SFC scoped styles

### crav-theme（Next.js 15 + Tailwind v4 + GSAP，Phase 2）

- 源站 `cravburgers.shop`，11 个 section（loader + nav + 8 个内容 + 2 个 curtain）
- 部署在 himaxlimited.myshopify.com，theme ID **155175616685**（2026-05-25 首部）
- 全项目复杂度集中在 **CSS cascade 战争**（Tailwind v4 @layer vs Lumia unlayered）和 **动效完整复刻**（8 类 source spec 反推）
- 关键踩坑（详细 [BUGS.md P4-01..10](BUGS.md#p4-01)，专题 [08-nextjs-porting.md](08-nextjs-porting.md)、[09-playwright-verify.md](09-playwright-verify.md)）：
  1. **Tailwind v4 `@layer utilities` 全军覆没**（P4-01）：Tailwind 编译产物把所有 utility 包在 layer 里，Lumia 的 unlayered h1 规则赢了 spec 更高的 .text-[30vw]。**与 specificity 无关**。修法：写 brace-counter unwrap 脚本剥离 layer 包装
  2. **自家 body `!important` 反伤 Tailwind**（P4-02）：在 `body.crav-index` 加 color/font-family `!important` 通过继承传染所有子元素，把 .text-red、.font-modak 压死
  3. **自家 `.crav-page h2` wrapper-prefix 反伤**（P4-03）：spec (0,1,1) 高于 Tailwind .leading-[.75] (0,1,0)，强加 `line-height: inherit` 让 h2 继承 .page-content 的 25px
  4. **Lumia `.page-content` 通过继承强加 15px/25px**（P4-04）：所有子元素如果没自己的 font-size/line-height 都被继承。修法：`.crav-page.page-content { font-size: inherit; line-height: normal }`
  5. **Next.js `_next/image` 输出 JPEG 丢 alpha**（P4-05）：透明 PNG/WebP 显示黑底。必须从 raw `/img/` `/img-webp/` 下载
  6. **Prettify HTML 让 char-split spans 出现 inline-block 空格**（P4-06）：CRAV 4 字母间各 ~118px 空格，整段 1265→1620px 溢出 viewport。HTML 层面去掉换行
  7. **`font-size: 0` collapse 技巧破坏 ScrollTrigger 测量**（P4-07）：trigger 元素 layout box 高度 0，onEnter 永不 fire
  8. **GSAP 付费 plugin vanilla 替代**（P4-08）：InertiaPlugin → CSS transition；MotionPathPlugin → SVG getPointAtLength；老 SplitText → 自己 split spans
  9. **Cursor rope 静止塌缩时序**（P4-09）：mousemove 同时做两件事 — 0.01s extend 段长到 8 + setTimeout(0) 排队 0.5s 渐变到 0。停止 → 队列里最后一次 collapse 完成 → 4 dots 重合到一点
  10. **Playwright 自验必须**（P4-10）：之前 push 完就报告，4 次明显错误用户截图后才发现。用 CDP `CSS.getMatchedStylesForNode` 找哪条规则在赢，逐 section scroll+screenshot

- **架构提炼**：
  - 11 个 Liquid section（loader / nav / hero / about / experience / cheesy-curtain / ingredients / map / cta / footer / exit-curtain）
  - 自制 JS `crav-app.js` ~450 行，replaces 源站 ~2MB Next.js + React + GSAP chunks。覆盖 8 类动效（pop / sticker peel / jelly bar / map plane motion / cursor rope / eye-follow / juggle balls / smooth scroll）
  - 本地化 assets：33 个 crav-* 文件（webp/png 透明素材 + woff2 字体 + 4 个 GSAP libs + CSS + JS）
  - CSS 文件：3 个（crav-app.css 56KB Tailwind 编译产物 unwrap layer / crav-extras.css 1.9KB sticker peel CSS / crav-base.css ~80 行 cascade 防御）

- **元教训**：
  - **Tailwind v4 + 母版主题部署的首要陷阱**是 @layer cascade，不是 specificity 战争。先 grep `@layer` 必查
  - **自家防御层 CSS 是反向 sabotage 的高发地**。每个 `.{prefix}-page X` 或 `body.{prefix}-index { Y !important }` 都要审 selector 是否提级伤 Tailwind，属性是否 inherited
  - **必须 Playwright 自验后才报告**。不省这一步
